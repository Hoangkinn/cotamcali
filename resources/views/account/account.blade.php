        ...
        <div class="page-header">
            <div class="page-block">
                <div class="page-header-title">
                    <h5 class="mb-0 font-medium">Nhân viên</h5>
                </div>
                <ul class="breadcrumb">
            ...
            <div class="col-span-12 xl:col-span-9 md:col-span-9">
                <div class="card table-card">
                    <div class="card-header">
                        ...
                        <a href="{{ route('addaccount') }}" class="btn btn-primary">+ Thêm mới</a>
                    ...
                    <div class="card-header">
                        ...
                        <form method="GET" action="{{ route('account') }}">
                                ...
                                <select name="status">
                                    ...
                                    <option value="1" {{ request('status') === '1' ? 'selected' : '' }}>Quản trị viên</option>
                                    <option value="2" {{ request('status') === '2' ? 'selected' : '' }}>Nhân viên</option>
                                    <option value="3" {{ request('status') === '3' ? 'selected' : '' }}>Giao hàng</option>
                                </select>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                @forelse($accountList as $index => $account)
                                <tr class="customer-row unread" 
                                    data-ma="{{ $account['Code'] ?? $newCode }}"
                                    data-ten="{{ $account['Name'] ?? '' }}"
                                    data-sdt="{{ $account['PhoneNumber'] ?? '' }}"
                                    data-diachi="{{ $account['Address'] ?? '' }}"
                                    data-email="{{ $account['Email'] ?? '' }}"
                                    data-ngaysinh="{{ !empty($account['DateOfBirth']) ? \Carbon\Carbon::parse($account['DateOfBirth'])->format('d/m/Y') : '' }}"
                                    data-gender="{{ $account['Gender'] == 1 ? 'Nam' : 'Nữ' }}"
                                    data-status="{{ $account['Status'] == 1 ? 'Hoạt động' : 'Ngừng' }}"
                                    >
                                    <td>{{ $index + 1 }}</td>
                                    <!-- <td><h6 class="mb-1">{{ $account['accountCode'] ?? '' }}</h6></td> -->
                                    <td><h6 class="text-muted">{{ $account['Code'] ?? $newCode }}</h6></td>
                                    <td><h6 class="text-muted">{{ $account['Name'] ?? '' }}</h6></td>
                                    <td><h6 class="text-muted">{{ $account['PhoneNumber'] ?? '' }}</h6></td>
                                    ...
                                    <td><h6 class="text-muted">{{ $account['Email'] ?? '' }}</h6></td>

                                    <td><h6 class="text-muted">@php
                                                                    $roles = [
                                                                ...
                                                                {{ $roles[$account['Roles']] ?? 'Không xác định' }}
                                                            </h6></td>

                                    <td>
                                        <button type="button" class="btn btn-sm btn-info btn-show-detail">Chi tiết</button>
                                        <a href="{{ route('editaccount', $account['Id']) }}" class="btn btn-sm btn-warning">Sửa</a>
                                        <form action="{{ route('account.destroy', $account['Id']) }}" method="POST" style="display:inline; padding:0">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr>
                                <td colspan="9" style="padding-top: 30px; text-align: right;">
                                    <div class="pagination">
                                        {{-- Nút trước --}}
                                        @if ($accountList->onFirstPage())
                                            <span class="btn disabled">◀</span>
                                        @else
                                            <a href="{{ $accountList->previousPageUrl() }}" class="btn">◀</a>
                                        @endif

                                        {{-- Các số trang --}}
                                        @foreach ($accountList->getUrlRange(1, $accountList->lastPage()) as $page => $url)
                                            @if ($page == $accountList->currentPage())
                                        ...
                                        {{-- Nút sau --}}
                                        @if ($accountList->hasMorePages())
                                            <a href="{{ $accountList->nextPageUrl() }}" class="btn">▶</a>
                                        @else
                                            <span class="btn disabled">▶</span>
                                        @endif
                                    </div>
                                    ...
                                    <div class="popup-body">
                                        <div class="popup-body">
                                            <table class="popup-table">
                                                <tr><td><strong>Mã nhân viên:</strong></td><td id="popupMa"></td></tr>
                                                <tr><td><strong>Tên nhân viên:</strong></td><td id="popupTen"></td></tr>
                                                <tr><td><strong>Email nhân viên:</strong></td><td id="popupEmail"></td></tr>
                                                <tr><td><strong>SĐT nhân viên:</strong></td><td id="popupSdt"></td></tr>
                                                <tr><td><strong>Địa chỉ:</strong></td><td id="popupDiachi"></td></tr>
                                                <tr><td><strong>Ngày sinh:</strong></td><td id="popupNgaysinh"></td></tr>
                                                <tr><td><strong>Giới tính:</strong></td><td id="popupGender"></td></tr>
                                                <tr><td><strong>Trạng thái:</strong></td><td id="popupStatus"></td></tr>
                                            </table>
...
document.querySelectorAll('.btn-show-detail').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
        ...
        document.getElementById('popupEmail').textContent = row.dataset.email || '';
        ...
        document.getElementById('popupNgaysinh').textContent = row.dataset.ngaysinh || '';
