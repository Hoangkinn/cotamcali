        ...
        <div class="page-header">
            <div class="page-block">
                <div class="page-header-title">
                    <h5 class="mb-0 font-medium">Sửa thông tin combo xuất ăn</h5>
                </div>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">Trang chủ</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('combo') }}">Combo</a></li>
                    <li class="breadcrumb-item" aria-current="page">Sửa thông tin combo xuất ăn</li>
                </ul>
            </div>
        </div>
        <!-- [ breadcrumb ] end -->
            ...
            <div class="col-span-12 xl:col-span-9 md:col-span-9">
                <div class="card table-card">
                    <div class="card-header">
                        <h5>Sửa thông tin combo xuất ăn</h5>
                    </div>
                    <div class="card-body">
                    <form id="comboForm" action="{{ route('combo.update', $combo['Id']) }}" method="POST" class="grid grid-cols-12 gap-6">
                        @csrf
                        @method('PUT')

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="ComboCode" class="form-label">Mã combo</label>
                            <input type="text" class="form-control" id="ComboCode" name="ComboCode" value="{{ $combo['ComboCode'] }}" readonly>
                        </div>

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="ComboName" class="form-label">Tên combo</label>
                            <input type="text" class="form-control" id="ComboName" name="ComboName" value="{{ $combo['ComboName'] }}" required>
                        </div>

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="TotalMeals" class="form-label">Tổng xuất ăn</label>
                            <input type="number" class="form-control" id="TotalMeals" name="TotalMeals" value="{{ $combo['TotalMeals'] }}">
                        </div>

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="NumberOfDate" class="form-label">Số ngày sử dụng xuất ăn</label>
                            <input type="number" class="form-control" id="NumberOfDate" name="NumberOfDate" value="{{ $combo['NumberOfDate'] }}">
                        </div>

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="Description" class="form-label">Ghi chú</label>
                            <textarea class="form-control" id="Description" name="Description">{{ $combo['Description'] }}</textarea>
                        </div>

                        <div class="col-span-12 xl:col-span-6 md:col-span-6">
                            <label for="Status" class="form-label">Trạng thái</label>
                            <select class="form-control" id="Status" name="Status" required>
                                <option value="1" {{ $combo['Status'] == 1 ? 'selected' : '' }}>Hoạt động</option>
                                <option value="0" {{ $combo['Status'] == 0 ? 'selected' : '' }}>Ngừng</option>
                            </select>
                        </div>

                        <div class="col-span-12">
                            <button type="submit" class="btn btn-primary">Cập nhật</button>
                            <a href="{{ route('combo') }}" class="btn btn-secondary">Hủy</a>
                        </div>
                    </form>
...
document.getElementById('comboForm').addEventListener('submit', function(event) {
    let totalMeals = parseInt(document.getElementById('TotalMeals').value);
    let numberOfDate = parseInt(document.getElementById('NumberOfDate').value);

    if (totalMeals < numberOfDate) {
        event.preventDefault(); // Ngăn submit
        alert("⚠️ Tổng xuất ăn không được nhỏ hơn Số ngày sử dụng xuất ăn.");
    }
});
</script>
<!-- [ Main Content ] end -->
