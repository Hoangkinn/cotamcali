    ...
    <div class="pc-content">
        ...
        <div class="grid grid-cols-12 gap-x-6">
            ...
            <div class="col-span-12 xl:col-span-3 md:col-span-6">
                <div class="card">
                    <div class="card-header !pb-0 !border-b-0">
                        <h5>Khách hàng cần quan tâm</h5>
                    </div>
                    <div class="card-body">
                        <div class="flex items-center justify-between gap-3 flex-wrap">
                            <h3 class="font-light flex items-center mb-0">
                            <i class="feather icon-arrow-down text-danger-500 text-[30px] mr-1.5"></i>
                                {{ $dashboard['CustomersToFollowUp'] ?? 0 }}
                            </h3>
                            <p class="mb-0">10%</p>
                        </div>
                        <div class="w-full bg-theme-bodybg rounded-lg h-1.5 mt-6 dark:bg-themedark-bodybg">
            ...
            <div class="col-span-12 xl:col-span-9 md:col-span-9">
                <div class="card table-card">
                    <div class="card-header">
                        <h5>Suất ăn cần giao hôm nay</h5>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    @foreach($GetTodayInfo as $item)
                                        <tr>
                                            <td>{{ $loop->iteration + ($GetTodayInfo->currentPage() - 1) * $GetTodayInfo->perPage() }}</td>
                                            <td>{{ ($item['CustomerName'] ?? '')}}</td>
                                            <td>{{ ($item['PhoneNumber'] ?? '') }}</td>
                                            <td>{{ ($item['Address'] ?? '') }}</td>
                                            <td>{{ ($item['Quantity'] ?? '') }}</td>
                                            <td>{{ ($item['MealsRemaining'] ?? '') }}</td>
                                    ...
                                    <tr>
                                        <td colspan="9" style="padding-top: 30px; text-align: right;">
                                            <div class="pagination">
                                                {{-- Nút trước --}}
                                                @if ($GetTodayInfo->onFirstPage())
                                                    <span class="btn disabled">◀</span>
                                                @else
                                                    <a href="{{ $GetTodayInfo->previousPageUrl() }}" class="btn">◀</a>
                                                @endif

                                                {{-- Các số trang --}}
                                                @foreach ($GetTodayInfo->getUrlRange(1, $GetTodayInfo->lastPage()) as $page => $url)
                                                    @if ($page == $GetTodayInfo->currentPage())
                                                ...
                                                {{-- Nút sau --}}
                                                @if ($GetTodayInfo->hasMorePages())
                                                    <a href="{{ $GetTodayInfo->nextPageUrl() }}" class="btn">▶</a>
                                                @else
                                                    <span class="btn disabled">▶</span>
                                                @endif
                                            </div>
